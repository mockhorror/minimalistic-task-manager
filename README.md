...
Pet-проект для практики QA Automation на Python. Планируем построить набор автотестов (UI, API, smoke/regression/e2e) вокруг воображаемого минималистичного таск-менеджера в эстетике soft shojo/pinterest — ежедневник с pastel-фонами, коробкой лёгких 2D-элементов и мотивационными сообщениями.
...
- `docs/testing.md` — стратегия тестирования, чеклисты smoke/regression/e2e и матрица переменных окружения.
- `resources/` — место для мок-данных, скриншотов UI, референсов эстетики.
- `prototype/` — интерактивный макет ежедневника (vanilla HTML/CSS/JS). Чтобы посмотреть локально:

```bash
cd prototype
python3 -m http.server 4173
# открой http://localhost:4173 в браузере
```

Можно также просто открыть `prototype/index.html` двойным кликом. Макет поддерживает смену тем (Pilates Princess / Matcha Village / Beach Episode), добавление задач, отметку “выполнено” и кнопку “Завершить день ✨”.
